...
class Client(object):
    ...
    def solve_inner(self, num_epochs=10, batch_size=7):
        ...
        Return:
            ...
            (bytes_write here count for the full model, without counting the communication reduction by sparsification)
        '''

        bytes_w = self.model.size
        #print("这个用来测试bytes_w的大小：{}".format(bytes_w))
        soln, comp = self.model.solve_inner(self.train_data, num_epochs, batch_size)
        bytes_r = self.model.size
        #print("这个用来测试bytes_r的大小：{}".format(bytes_r))
