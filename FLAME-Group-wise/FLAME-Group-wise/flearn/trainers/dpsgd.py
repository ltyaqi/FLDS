...
class Server(BaseFedarated):
    ...
    def __init__(self, params, learner, dataset):
        ...
        self.sigma = (2*self.norm/(self.epsilon+0.2)) * math.sqrt(2 * math.log(1.25/self.delta))
    ...
    def local_process(self, flattened,clip_vector):
        for i in range(len(flattened)):
            flattened[i] = np.clip(flattened[i],-clip_vector[i],clip_vector[i])
        return flattened
