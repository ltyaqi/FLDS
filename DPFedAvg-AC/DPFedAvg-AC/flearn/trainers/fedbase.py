...
class BaseFedarated(object):
	...
	random.shuffle(self.clients)
                ...
                for csoln in csolns:
                    ...
                    tmp = []
                    processed_update,cliped_norm = self.local_process(flattened)
                    teep = cliped_norm/(2*self.norm)
                    if teep > 1:
                        cliped_norm = 2*self.norm
                    sum_cliped_norm.append(cliped_norm)
                ...
                self.latest_model = [u -(self.learning_rate*v) for (u, v) in zip(self.latest_model, self.server_process(csolns_new))]
                self.client_model.set_params(self.latest_model)
                if (count_iter == 0 or iter == (len(self.clients) - self.clients_per_round)):
                    sum_norm = np.sum(sum_cliped_norm)
                    print( "sum_norm is:{}".format( sum_norm))
                    self.norm = abs(1.9/len(selected_clients)*(sum_norm+np.random.normal(loc=0,scale=2*2.5*self.norm)))
