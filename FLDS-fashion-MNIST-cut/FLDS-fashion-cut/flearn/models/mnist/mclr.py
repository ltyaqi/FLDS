...
class Model(object):
    ...
    def __init__(self, num_classes, optimizer, seed=1):
        ...
        with self.graph.as_default(): #设置为默认图
            tf.set_random_seed(123 + seed) #随机数种子
    ...
    def create_model(self, optimizer):
        ...
        features = tf.placeholder(tf.float32, shape=[None, 784], name='features') #参数
        labels = tf.placeholder(tf.int64, shape=[None, ], name='labels') #这里的labels应当为训练集的类型
        ...
        grads_and_vars = optimizer.compute_gradients(loss) #计算梯度程度
