...
def process_grad(grads):
    ...
    Return:
        a flattened grad in numpy (1-D array)
    '''
    # print("查看grads的类型：{}".format(type(grads)))
    # print("查看grads的值：{}".format(grads))
    # print("查看grads的长度：{}".format(len(grads)))
    # print("查看客户端的梯度：{},{}".format(grads[0].shape,grads[1].shape))
    client_grads = grads[0]

    for i in range(1, len(grads)):
        client_grads = np.append(client_grads, grads[i])  # output a flattened array
    # print("查看更新后的值：{}".format(client_grads.shape))
    # print("查看clients_grads的值：{}".format(client_grads))
    return client_grads
