...
class Server(BaseFedarated):
    ...
    def local_process(self, flattened):
        processed_update,L2_norm = clip(flattened, self.norm)  # L_2 clipping
        return processed_update,L2_norm
    ...
    def server_process(self, solns):  # DP equals to DDP with the same centralized gaussian noise (privacy level)
        ...
        for i, _ in enumerate(base):
            print("输出范数：{}".format(self.norm))
            sigma = (2 * self.norm / self.epsilon) * math.sqrt(2 * math.log(1.25 / self.delta))
            print("输出噪音方差：{}".format(sigma))
            base[i] = base[i] + np.random.normal(0, sigma, base[i].shape)
