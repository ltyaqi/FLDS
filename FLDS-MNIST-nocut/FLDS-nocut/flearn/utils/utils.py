...
def generater_random_matrix(clients,dimensions,sum_sample_clients,sum_sample_dimensions,epsilon,clip):
    """
    算法步骤：
    ...
    for i in range(random_choices_matrixes.shape[0]):
        for j in range(random_choices_matrixes.shape[1]):
            if random_choices_matrixes[i][j] == 1:
                np.random.seed()
                noise_matrix[i][j] = one_laplace(epsilon,sensitivity=2*clip)
    #查看矩阵的值
